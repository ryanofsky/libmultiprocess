    ...
    typename std::enable_if<std::is_same<decltype(output.get()), Context::Builder>::value>::type* enable = nullptr)
{
    ...
    // Also store a reference to the Thread::Client in callback_threads map so
    ...
    // dedicated worker thread to run function calls from this thread. Store a
    // reference to the Thread::Client function it returns in the
    // request_threads map.
