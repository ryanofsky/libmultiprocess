...
||||||| parent of aa49a11 (proxy-io: manage Connection lifetime with shared_ptr)
...
>>>>>>> aa49a11 (proxy-io: manage Connection lifetime with shared_ptr)
...
||||||| parent of 17b17f3 (doc: scope Connection cleanup callbacks to per-thread map entries)
...
>>>>>>> 17b17f3 (doc: scope Connection cleanup callbacks to per-thread map entries)
...
||||||| parent of 5ad8807 (proxy-io: keep client capability handles across disconnect)
            // Remove disconnect callback on cleanup so it doesn't run and try
            // to access this object after it's destroyed. This call needs to
            // run inside loop->sync() on the event loop thread because
...
=======
>>>>>>> 5ad8807 (proxy-io: keep client capability handles across disconnect)
    ...
    if (destroy_connection) {
        m_context.loop->sync([&] {
            ...
            Connection* connection = m_context.connection.get();
