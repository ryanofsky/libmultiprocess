...
struct ProxyClient<Thread> : public ProxyClientBase<Thread, ::capnp::Void>
{
    ...
    //! object gets destroyed before the Connection, the cleanup will be
    //! unregistered in the destructor and never run.
...
using MakeThreadFn = std::function<Thread::Client()>;

// Retrieve or create ProxyClient<Thread> object associated with this
// connection. Return map iterator and inserted bool.
std::tuple<ConnThread, bool> SetThread(ConnThreads& threads, std::mutex& mutex, Connection* connection, MakeThreadFn make_thread);
