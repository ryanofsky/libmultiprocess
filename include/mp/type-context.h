    ...
    return server.m_context.connection->m_threads.getLocalServer(thread_client)
        .then([&server, invoke = kj::mv(invoke), req](const kj::Maybe<Thread::Server&>& perhaps) mutable {
            ...
            KJ_IF_MAYBE (thread_server, perhaps) {
                ...
                // Detect when getLocalServer call fails. The serverInvoke
                // function will print the full exception object after this, but
                // it is worth printing an additional message here because a
                // failure here is probably caused by failing to pass a
                // ThreadMap object from the server to the client before this
                // call. Specifically what happens if there was not a prior IPC
                // call returning a ThreadMap is that the client's
                // Connection::m_thread_map member will be null when it makes
                // this call, and when the client makes a
                // m_thread_map.makeThreadRequest() as part of this IPC, the
                // request will appear to succeed due to Cap'n Proto's request
                // pipelining, but actually return a null Thread::Client
                // capability, and when that capability is passed to
                // getLocalServer it will fail with an error here "(remote):0:
                // failed: remote exception: Called null capability" that's
                // pretty opaque, so good to print a more specific message.
                server.m_context.loop->log()
