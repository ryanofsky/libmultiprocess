...
void EventLoopRef::reset(bool relock) MP_NO_TSA
{
    if (auto* loop{m_loop}) {
        ...
        if (loop->done()) {
            ...
            // Capture m_post_writer before releasing the lock. After unlocking,
            // the event loop can read from wait_stream, see done(), exit loop(),
            // and the EventLoop object (including m_post_writer and the mutex)
            // may be destroyed. Accessing m_post_writer through a pointer
            // captured while holding the lock is safe because this write is what
            // wakes the event loop — the loop can't exit (and the EventLoop can't
            // be destroyed) until after the write. For the same reason, do not
            // try to relock after writing unless explicitly requested.
