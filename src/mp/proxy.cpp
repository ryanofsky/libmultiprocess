...
{
    // Assert connection destructor is always called on the event loop thread.
    // If this is a local disconnect, it will trigger I/O, so this needs to run
    // on the event loop thread, and if there was a remote disconnect, this is
    // called by an onDisconnect callback directly from the event loop thread.
