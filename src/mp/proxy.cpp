...
CleanupIt Connection::addSyncCleanup(std::function<void()> fn)
{
    ...
    // added as client objects are created, it natural to clean up objects in
    ...
    // single batch, and they only reset the connection pointers in the client
    // objects, without actually deleting the client objects.
...
void Connection::addAsyncCleanup(std::function<void()> fn)
{
    ...
    // already destroys these objects in LIFO order, we should preserve this
    // ordering, and add cleanup callbacks to the end of the list so they can be
    // run starting from the beginning of the list.
    //
    // In bitcoin core, running these callbacks is particularly important for
    // the wallet process, because it uses shared_ptr and requires
    // Chain::Notification objects owned by the node process to be destroyed
    // before the WalletLoader objects owned by the node process, otherwise
    // shared pointers counts of the CWallet objects will not be 1 when
    // WalletLoader destructor runs and it will wait forever for them to be
    // released.
